%&tex
\chapter{Disentanglement of Shape and Appearance}
\begin{comment}
...
\section{Disentangling Pose and Appearance}\label{sec:poseandappearance}
		...
		\paragraph{Deep Fashion} \cite{liu16deepfashion, liu16deepfashionwild} consists of ca. 53k in-shop clothes images in high-resolution of $256 \times 256$. We selected the images which are showing a full body (all keypoints visible, measured with the pose estimator by \cite{cao17affinityfield}) and used the provided train-test split. For comparison with Esser \etal \cite{esser18} we used their published code. \todo{experiment with boxes: dataset}

	\subsection{Person Re-Identification}

		Person re-identification (ReID) is a research field on its own \eg \cite{almazan18reidtowards, zheng16reidcnn}, the goal being to learn a similarity metric for a persons appearance, invariant to a persons posture and the image viewpoint.
...
\section{Part-wise Disentanglement}\label{sec:partwise}
	...
	Own Dataset: Move KP, Fig.~\ref{fig:movekp}
	\begin{figure}[htp]
		\centering
...
\begin{comment}
\section{Follow-Up}\label{sec:followup}
	\begin{itemize}
