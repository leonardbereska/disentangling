...
\section{Learning from Data}
	...
	\subsection{Supervised}\label{sec:supervised}
		...
		\todo{example for landmarks}
		Recent advances, in particular the effectiveness of neural network models (cf. sec. \ref{sec:neuralnetworks}) on big datasets, have led to huge progress on problems that can be formulated as regression or classification. On many traditional computer vision problems, such as \eg object recognition, image classification or human pose estimation, machines are now performing on a superhuman level; hence, many supervised problems are considered to be solved problems\todo{ cite here}.
...
\section{Generative Models}\label{sec:genmodel}
	...
	\subsection{Adversarial Formulations}\ref{sec:adversarial}
		...
		There are many variants and extensions to this basic principle of learning with an adversarial principle. For example one can learn a discriminator on for a set of image patches \todo{cite CoGAN}
...
\section{Disentangling Representations}\ref{sec:disentangled}
	...
	\begin{quote}
		{Disentangle as many factors as possible, discarding as little information about the data as is practical.} - Y. Bengio, A. Courville and P. Vincent \todo{cite{bengio}}
	\end{quote}
	...
	\subsection{Shape and Appearance}
		\note{especially difficult because of domain}
