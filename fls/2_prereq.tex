...
\section{Disentangling Representations}\label{sec:disentangled}
	...
	\subsection{Disentangling by Equivariance and Invariance}
		...
		\note{In general, these factors can interact in complicated ways to finally result in an image (cf. Fig. \ref{}). Here, we only consider the case where multiple independent factors each have an influence (cf. Fig. \ref{fig:inferlevel}).}
		...
		A disentangled representation should simultaneously fullfill equivariance and invariance: A change in $\mathbf{z_i}$ should: \emph{i)} \textit{equivariantly} change in the abstract representational factor $\mathbf{\hat z_i}$, \emph{ii)} while leaving the other factors $\mathbf{\hat z_j}, j\neq i$, that represent other causes, \textit{invariant}.
		\begin{figure}[ht]
			\centering
...
\section{Theoretical Impediments from Causality}\label{sec:causality}
	...
	Learning a disentangled representation is then causal inference from image data.
	In accordance with the causal literature, we can make statements about the type of knowledge, that can be gained by learning from "raw" image data, raw data meaning images $x$ sampled from $p(x)$.
	...
	\subsection{Causal Learning}
		...
		\end{figure}
		\todo{intro to causal learning}
		\note{Causal learning is a hard problem: instead of only learning statistical measures from data, model also needs to be learned (\cite{peters17elements})}
		...
		\note{ladder of causation: association, intervention, counter-factual}


	\subsection{Unsupervised Disentangling requires Interventions or Model Assumptions}
		...
		This means that "purely" unsupervised disentangling, \ie estimating $\mathbf{\hat z_i}$ from samples $x \sim p(x)$, is impossible.
		\note{A rigorous argument for this can be found in todo{cite schoelkopf}}% for estimation of causal factors "raw data" insufficient -> need interventional data or model assumptions.
		% so far fitting curve p(x) to data manifold
		Current machine learning operates mostly on the level of association, estimating (comlex) correlations from raw data.
...
%
	...
	\subsection{Image Transformations are Interventions}
		\note{we harness intervention}
		$p(x| do(a), b)$
		\note{in computer vision an intervention is an image transformation if ..}

	\subsection{Analysis-by-Synthesis as Model Assumptions}
		% Inverse graphics
